function [Pratio, eff, Mrel1, DFr, utr, Cx2, R, phi2, criteria]=subsonic(x)
...
load ('RotorLoss.mat'); DFr_RL=RotorLoss(:,1); wrcos_RL=RotorLoss(:,2); f_RL=fit(DFr_RL,wrcos_RL,'smoothingspline');
load ('RotorLoss10.mat'); DFr_RL10=RotorLoss10(:,1); wrcos_RL10=RotorLoss10(:,2); f_RL10=fit(DFr_RL10,wrcos_RL10,'smoothingspline');
wrcos=0.7.*f_RL(DFr)+0.3.*f_RL10(DFr);
wr=wrcos.*((2.*soldr)./(cosd(beta2d)));
...
P2=Ptrel2./((1+((G-1)./2).*Mrel2.^2)).^(G./(G-1));
...
tol=0.001;
load ('RotorLoss.mat'); DFs_SL=RotorLoss(:,1); wscos_SL=RotorLoss(:,2); 
f_SL=fit(DFs_SL,wscos_SL,'smoothingspline');
for j=1:length(DFsassumed)
    ...
    Ams =(A2+A3)./2;                                  %Area Mean
    rms=sqrt((Ams.*(1+zetas))./(4.*pi.*(1-zetas)));   %Radius Mean
...
if((Mrel1>=0.6)&&(Mrel1<=0.77)&&(DFr>=0.3)&&(DFr<=0.47)&&(utr<=300)&&(Cx2>=100)&&(Cx2<=150)&&(R<=1)&&(R>=0.5)&&(phi2>=0.4)&&(phi2<=0.6))
 % Satisfied 
    criteria=min(-(0.6.*(Pratio)+0.4.*(eff)));
else
    criteria=455567;
